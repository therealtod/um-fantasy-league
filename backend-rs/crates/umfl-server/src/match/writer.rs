...
pub async fn insert(conn: &mut PgConnection, m: &TournamentMatchWrite) -> sqlx::Result<i64> {
    let id = sqlx::query_scalar!(
        "insert into tournament_matches (tournament_id, round, played_at, external_link)
...
pub async fn update(conn: &mut PgConnection, m: &TournamentMatchWrite) -> sqlx::Result<()> {
    ...
    // `match_game_participants` hangs off `match_games`, not off the match, so it
    // goes first and by hand -- deleting the games would cascade it away, but
    // only after this statement has already named the rows.
    sqlx::query!(
        "delete from match_game_participants
         where game_id in (select id from match_games where match_id = $1)",
        id
    )
    .execute(&mut *conn)
    .await?;
    ...
    sqlx::query!("delete from match_games where match_id = $1", id)
        .execute(&mut *conn)
        .await?;
    sqlx::query!("delete from match_participants where match_id = $1", id)
        .execute(&mut *conn)
        .await?;
    sqlx::query!("delete from hero_bans where match_id = $1", id)
        .execute(&mut *conn)
        .await?;
    sqlx::query!("delete from match_hero_picks where match_id = $1", id)
        .execute(&mut *conn)
        .await?;
...
pub async fn delete(conn: &mut PgConnection, match_id: i64) -> sqlx::Result<()> {
    sqlx::query!("delete from tournament_matches where id = $1", match_id)
        .execute(conn)
        .await?;
    Ok(())
}
...
///
/// `tournament_id` is denormalised off the root purely so `match_games` can
